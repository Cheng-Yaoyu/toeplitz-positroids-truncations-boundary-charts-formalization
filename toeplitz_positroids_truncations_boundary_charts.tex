...
\]
be a finite real Toeplitz matrix.  The AIM problem asks for the possible zero--nonzero patterns of minors of a totally nonnegative Toeplitz matrix, or, at the maximal-minor level, for the positroid cells that meet the Toeplitz locus \cite{AIM}.  The distinction between Grassmannian nonnegativity and all-minor total nonnegativity is essential: throughout this paper a \emph{Toeplitz positroid} is represented by a full-row-rank Toeplitz matrix whose minors of every order are nonnegative.
...
\begin{equation}\label{eq:dual-JT}
 ...
 =s_{\lambda'/\mu'}(Y)
\end{equation}
for suitable partitions $\mu\subseteq\lambda$ of length at most $k$; explicitly,
\[
...
\[
 ...
 =(x_1\cdots x_r)^t=1.
\]
Equivalently, recurrence \eqref{eq:h-recurrence} advances these cofactor blocks by a companion matrix of determinant one.  Since the initial block is unit upper triangular, every cofactor determinant is one.
...
The preceding theorems are local realization results.  We next record two global families with exact support formulas.
...
The conjecture is consistent with the complete low-rank classification, the paving and loop-paving classifications, and the finite Edrei and finite-band families.  It is stronger than the assertion that the matroid is merely a positroid, because it singles out the displayed linear order rather than the ambient cyclic order.
