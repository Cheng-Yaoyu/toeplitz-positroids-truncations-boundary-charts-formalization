...
private lemma finset_card_filter_get_eq_count {α : Type*} [DecidableEq α] (L : List α) (x : α) :
  ...
  | cons a as ih =>
    ...
    calc
      (Finset.filter (fun i : Fin (as.length + 1) => (a :: as).get i = x)
          Finset.univ).card =
          (if a = x then 1 else 0) +
            (Finset.filter (fun i : Fin as.length => as.get i = x)
              Finset.univ).card := key
      _ = List.count x as + (if (a == x) = true then 1 else 0) := by
        rw [ih]
        by_cases ha : a = x
...
theorem monomialExp_totalDegree [Nontrivial R] (a : Fin N → ℕ) :
  ...
  change ∑ x ∈ (Function.support a).toFinset, a x = ∑ i, a i
  have hsupport : (Function.support a).toFinset =
      Finset.univ.filter (fun x ↦ a x ≠ 0) := by
    ext x
    simp [Function.mem_support]
  rw [hsupport]
  rw [← Finset.sum_filter_add_sum_filter_not (s := Finset.univ) (p := fun x => a x = 0)]
  have hzero : ∑ x ∈ Finset.univ.filter (fun x ↦ a x = 0), a x = 0 := by
