...
private lemma sign_revPerm_eq' (n : ℕ) :
  ...
  have h3 : ∏ x : Fin n, (-1 : ℤˣ) ^ (n - 1 - x.val) =
    ...
    exact Finset.prod_pow_eq_pow_sum Finset.univ
      (fun x : Fin n ↦ n - 1 - x.val) (-1 : ℤˣ)
  calc
    ∏ x : Fin n, (-1 : ℤˣ) ^ (n - 1 - x.val) =
        (-1 : ℤˣ) ^ (∑ x : Fin n, (n - 1 - x.val)) := h3
    _ = (-1 : ℤˣ) ^ (n * (n - 1) / 2) := by rw [sum_sub_val_eq']
...
private lemma units_neg_one_pow_coe' (k : ℕ) :
  ...
  | zero => simp
  | succ k ih =>
      rw [show (-1 : ℤˣ) ^ (k + 1) = (-1 : ℤˣ) ^ k * (-1) by
        exact pow_succ _ _]
      simp only [Units.val_mul, Int.cast_mul, ih, Units.val_neg,
        Units.val_one, Int.cast_neg, Int.cast_one]
      exact (pow_succ (-1 : MvPolynomial (Fin N) R) k).symm
...
private lemma freeKCols_card {lam mu : Fin N → ℕ} (T : Tableau lam mu) 
  ...
  · -- The filter sets are equal (up to instance differences)
      ...
      rw [show skewYoungDiagram_fintype lam mu = this by
...
/-- Column-strict preservation for Bender-Knuth involution.
    ...
    This lemma supplies the previously missing column-strict proof in
    benderKnuth_involutive when partition
...
- Involutivity: Uses the fully checked theorem
  `benderKnuthPrefixMatching_involutive_stembridge`
...
/-- stembridgeInvolutionMatching is an involution on non-Yamanouchi tableaux.
    ...
    This theorem uses the fully checked theorem
    `benderKnuthPrefixMatching_involutive_stembridge`. -/
...
lemma alternant_mul_skewSchurPoly_eq_sum {lam mu : Fin N → ℕ}
  ...
  -- Factor out the smul
  have hsmul :
      (∑ x : {T : Tableau lam mu // IsSemistandard T},
        Equiv.Perm.sign σ •
          (xPow (α ∘ σ + contentTableau x.val ∘ σ) :
            MvPolynomial (Fin N) R)) =
      Equiv.Perm.sign σ •
        ∑ x : {T : Tableau lam mu // IsSemistandard T},
          (xPow (α ∘ σ + contentTableau x.val ∘ σ) :
    ...
    symm
    simpa only using (Finset.smul_sum
      (s := (Finset.univ : Finset {T : Tableau lam mu // IsSemistandard T}))
      (r := Equiv.Perm.sign σ)
      (f := fun x ↦ (xPow (α ∘ σ + contentTableau x.val ∘ σ) :
        MvPolynomial (Fin N) R)))
  rw [hsmul]
  congr 1
